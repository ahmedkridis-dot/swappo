...
serve(async (req) => {
  ...
  // getUser(jwt): an Edge Function has no persisted session, so the token
  // must be passed explicitly — without it recent supabase-js versions throw
  // AuthSessionMissingError even for a perfectly valid user JWT.
